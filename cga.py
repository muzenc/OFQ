...
parser.add_argument(
    "--model", default="resnet101", type=str, metavar="MODEL", help='Name of model to train (default: "countception"'
)
parser.add_argument(
    "--pretrained",
    action="store_true",
...
parser.add_argument(
    "--img-size", type=int, default=None, metavar="N", help="Image patch size (default: None => model default)"
)
parser.add_argument(
    "--input-size",
    default=None,
...
parser.add_argument(
    "-b", "--batch-size", type=int, default=32, metavar="N", help="input batch size for training (default: 32)"
)
parser.add_argument(
    "-vb",
...
parser.add_argument(
    "--warmup-lr", type=float, default=0.0001, metavar="LR", help="warmup learning rate (default: 0.0001)"
)
parser.add_argument(
    "--min-lr", type=float, default=1e-5, metavar="LR", help="lower lr bound for cyclic schedulers that hit 0 (1e-5)"
)
parser.add_argument("--epochs", type=int, default=200, metavar="N", help="number of epochs to train (default: 2)")
parser.add_argument(
    "--epoch-repeats",
    type=float,
    default=0.0,
...
parser.add_argument(
    "--warmup-epochs", type=int, default=3, metavar="N", help="epochs to warmup LR, if scheduler supports"
)
parser.add_argument(
    "--cooldown-epochs",
    type=int,
...
parser.add_argument(
    "--patience-epochs", type=int, default=10, metavar="N", help="patience epochs for Plateau LR scheduler (default: 10"
)
parser.add_argument(
...
parser.add_argument(
    "-j", "--workers", type=int, default=4, metavar="N", help="how many training processes to use (default: 1)"
)
parser.add_argument(
    "--save-images",
    action="store_true",
...
parser.add_argument("--wq-enable", action="store_true", default=False, help="enable weight quantization")
parser.add_argument("--wq-mode", default="LSQ", type=str, help="weight quantization mode")
parser.add_argument("--wq-bitw", default=2, type=int, help="weight quantization bit width")
parser.add_argument("--wq-per-channel", default=False, action="store_true", help="per channel weight quantization")
parser.add_argument("--wq-asym", action="store_true", default=False, help="asymmetric quantization for weight")
parser.add_argument(
    "--wq_clip_learnable", action="store_true", default=False, help="learbable step size / clip value for weight quant"
)
parser.add_argument("--aq-enable", action="store_true", default=False, help="enable act quantization")
parser.add_argument("--aq-mode", default="lsq", type=str, help="act quantization mode")
parser.add_argument("--aq-bitw", default=2, type=int, help="act quantization bit width")
parser.add_argument("--aq-per-channel", default=False, action="store_true", help="per channel activation quantization")
parser.add_argument(
    "--aq_clip_learnable",
    action="store_true",
    default=False,
...
parser.add_argument("--qmodules", type=str, nargs="+", default=None, metavar="N", help="Quantized modules")
parser.add_argument(
    "--replace-ln-by-bn", action="store_true", default=False, help="whether to use bn instead of layernorm"
)
parser.add_argument("--use-kd", action="store_true", default=False, help="whether to use kd")
parser.add_argument("--use-token-kd", action="store_true", default=False, help="whether to use token level kd")
parser.add_argument("--kd-alpha", default=1.0, type=float, help="KD alpha, soft loss portion (default: 1.0)")
parser.add_argument(
    "--teacher", default="resnet101", type=str, metavar="MODEL", help='Name of teacher model (default: "countception"'
)
parser.add_argument(
    "--teacher-checkpoint",
    default="",
...
parser.add_argument("--quant-teacher", action="store_true", default=False, help="whether to quantize the teacher")
parser.add_argument(
    "--use-distill-head", action="store_true", default=False, help="whether to use two separate heads for distillation"
)
parser.add_argument(
    "--use-layer-scale", action="store_true", default=False, help="whether to use scaler for attention and mlp"
)
parser.add_argument("--use-skip", action="store_true", default=False, help="whether to use skip for connection")
parser.add_argument("--use-relu", action="store_true", default=False, help="whether to use relu for the model")
parser.add_argument("--kd-type", type=str, default="last", help="whether to match the last or all the embeddings")

parser.add_argument("--gpu_id", default=0, type=int, help="gpu id to use for single gpu training")
parser.add_argument("--model_type", type=str, default="deit", help="model type to quantize: deit or swin")
parser.add_argument("--quantized", action="store_true", default=False, help="whether to quantize model")
...
parser.add_argument(
    "--act_layer", type=str, default="gelu", help="activation function to use for the fc1 output of MLP"
)  # ACT_LAYER_MAPPINGS, ['relu', 'gelu', 'prelu', 'rprelu']
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="false = does not use pretrained weight to initialize",
)
...
parser.add_argument(
    ...
    type=float,
    default=0.005,
    help="Boubdary Range for CGA, usually 0.005 or 0.007 yield the best result",
)
...
def get_qat_model(model, args):
    ...
    ACT_LAYER_MAPPINGS = {"relu": nn.ReLU, "gelu": nn.GELU, "prelu": nn.PReLU, "rprelu": "rprelu", "None": "None"}
    ...
    for m in args.qmodules:
        ...
        qconfigs[m] = {
            "weight": wcfg,
            "act": acfg,
            "q_attn_dropout": args.apply_q_attn_dropout,
            "act_layer": ACT_LAYER_MAPPINGS[args.act_layer],
        }
    if args.model_type == "deit":
        qat_model = replace_module_by_qmodule_deit(
            model,
            qconfigs,
            pretrained_initialized=args.pretrained_initialized,
            qk_reparam=args.qk_reparam,
            qk_reparam_type=args.qk_reparam_type,
            boundaryRange=args.boundaryRange,
        )
    elif args.model_type == "swin":
        qat_model = replace_module_by_qmodule_swin(
            model,
            qconfigs,
            pretrained_initialized=args.pretrained_initialized,
            qk_reparam=args.qk_reparam,
...
def freeze_outside_boundary_weight_idx(cur_weight, num_bits, boundaryRange=0.005):
    ...
    b4_round, round, min, max = get_min_max_round(
        weights=cur_weight, scaling_factor=scaling_factor, clip_val=clip_val, num_bits=num_bits
    )

    for i in np.arange(
        start=min.detach().cpu(), stop=max.detach().cpu()
    ):  # 0.5 - boundaryRange < x < 0.5 + boundaryRange
        within_boundary = ((b4_round - i) <= (0.5 + boundaryRange)) * (
            (b4_round - i) >= (0.5 - boundaryRange)
        )  # idx of # 0.5 - boundaryRange < x < 0.5 + boundaryRange
...
def main(local_rank, args):
    ...
    if args.distributed:
        ...
        torch.distributed.init_process_group(
            backend="nccl",
            init_method="tcp://localhost:" + args.tcp_port,
    ...
    if args.jsd:
        ...
        if args.kd_hard_and_soft == 0:
            ...
            args.kd_hard_and_soft == 1
        ):  ## this loss function will automatically apply soft cross entropy if the label probability is given
...
def train_one_epoch(
    ...
    teacher_type="deit",
    log_wandb=False,
    annealing_schedule_for_dampen=None,
                ...
                if args.model_type == "swin":
                    for k, v in model.named_modules():
                        if (
                            ...
                            freeze_idx = freeze_outside_boundary_weight_idx(
                                v.weight, args.wq_bitw, boundaryRange=args.boundaryRange
                            )
...
def validate(
    model, loader, loss_fn, args, amp_autocast=suppress, log_suffix="", model_type="deit", epoch=0, log_wandb=False
):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
...
if __name__ == "__main__":
    ...
    mp.spawn(main, args=((args, args_text),), nprocs=int(os.environ["WORLD_SIZE"]), join=True)
